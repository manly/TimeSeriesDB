...
namespace TimeSeriesDB.DataStreams.Readers
{
    ...
    public sealed class DataStreamReader_Int64_LSB : DataStreamReaderWrapperBase_Complex<long, ulong> {
        public DataStreamReader_Int64_LSB() : base(new DataStreamReader_UInt64_LSB(), sizeof(ulong)) { }

        protected override long Convert(ulong value) {
            return this.InternalConvert(value);
        }

        protected override void ConvertFromBuffer(long[] values, int offset, int count) {
            int read = 0;
            while(count >= 16) {
