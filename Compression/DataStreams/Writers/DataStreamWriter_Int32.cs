...
namespace TimeSeriesDB.DataStreams.Writers
{
    ...
    public sealed class DataStreamWriter_Int32 : DataStreamWriterWrapperBase_Complex<int, uint>, IResumableDataStreamWriter {
        public DataStreamWriter_Int32() : base(new DataStreamWriter_UInt32(), sizeof(uint)) { }

        protected override uint Convert(int value) {
            return this.InternalConvert(value);
        }

        protected override void ConvertToBuffer(int[] values, int offset, int count) {
            int write = 0;
            while(count >= 16) {
